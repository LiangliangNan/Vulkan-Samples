...
bool ApiVulkanSample::prepare()
{
	...
	if (get_window().get_window_mode() != vkb::Window::Mode::Headless)
	{
	...
	gui = std::make_unique<vkb::Gui>(*this, get_window(), /*stats=*/nullptr, 15.0f, true);
...
void ApiVulkanSample::submit_frame()
{
	if (render_context->has_swapchain())
	{
		...
		if (device->is_extension_supported(VK_KHR_DISPLAY_SWAPCHAIN_EXTENSION_NAME) &&
		    get_window().get_display_present_info(&disp_present_info, width, height))
		{
